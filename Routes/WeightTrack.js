...
router.post("/addweightentry", authTokenHandler, async (req, res) => {
  ...
  if (!date || !weightInKg) {
    return res
      .status(400)
      .json(createResponse(false, "Please provide date and weight"));
  }
...
router.post("/getweightbydate", authTokenHandler, async (req, res) => {
  ...
  if (!date) {
    ...
    return res.json(
      createResponse(true, "Weight entries for today", user.weight)
    );
  }
...
router.post("/getweightbylimit", authTokenHandler, async (req, res) => {
  ...
  if (!limit) {
    ...
    return res.json(
      createResponse(
        true,
        `Weight entries for the last ${limit} days`,
        user.weight
      )
    );
  }
...
router.get("/getusergoalweight", authTokenHandler, async (req, res) => {
  ...
  const currentWeight =
    user.weight.length > 0 ? user.weight[user.weight.length - 1].weight : null;
  const goalWeight =
    22 * (user.height[user.height.length - 1].height / 100) ** 2;

  res.json(
    createResponse(true, "User goal weight information", {
      currentWeight,
      goalWeight,
    })
  );
