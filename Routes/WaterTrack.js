...
router.post("/addwaterentry", authTokenHandler, async (req, res) => {
  ...
  if (!date || !amountInMilliliters) {
    return res
      .status(400)
      .json(createResponse(false, "Please provide date and water amount"));
  }
...
router.post("/getwaterbydate", authTokenHandler, async (req, res) => {
  ...
  if (!date) {
    ...
    return res.json(
      createResponse(true, "Water entries for today", user.water)
    );
  }
...
router.post("/getwaterbylimit", authTokenHandler, async (req, res) => {
  ...
  if (!limit) {
    ...
    return res.json(
      createResponse(
        true,
        `Water entries for the last ${limit} days`,
        user.water
      )
    );
  }
